...
module.exports = (dbInject) => {
    ...
    const updatePasswordUser = async (body) => {
        const dataPassword = {}
        dataPassword.password = await bcrypt.hash(body.password.toString(), 5)
        console.log({body})
        await db.updateItem(TABLE, 'password', dataPassword.password, body.id)
    }
